...
let counter = setInterval(() => {
    ...
    let goal = new Date('Apr 30, 2022 19:17:00').getTime()

    let now = new Date().getTime()

    let timer = goal - now

    let months = Math.floor(timer / (1000 * 60 * 60 * 24 * 30))
    let days = Math.floor(timer % (1000 * 60 * 60 * 24)/ 1000 / 60 / 60 / 24);
    ...
    five.textContent = months < 10 ? `0${months}` : months;
}, 1000);
